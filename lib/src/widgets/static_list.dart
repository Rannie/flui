...
enum FLStaticListCellAccessoryType { accNone, accCheckmark, accDetail, accSwitch, accCheckBox }
...
class FLStaticItemData {
  FLStaticItemData({
    ...
    this.customizeContent,
          ...
          if (cellType == FLStaticListCellType.normal && (leading == null && title == null && subtitle == null)) {
          ...
          if (cellType == FLStaticListCellType.customization && customizeContent == null) {
            throw AssertionError(
              'Must specify the content when cells type is customization',
            );
          }
...
class FLStaticListView extends StatelessWidget {
  ...
  int _preCalItemCount() {
    ...
    for (int i = 0; i < sections.length; i += 1) {
      ...
      int itemCount = sectionData.itemList?.length ?? 0;
      count += itemCount;
    }
    return count;
  }

  Widget _buildSectionHeader(FLStaticSectionData sectionData, ThemeData themeData, BuildContext context) {
  ...
  Widget _buildItemCell(FLStaticItemData itemData, ThemeData themeData) {
    final Text? titleText = itemData.title == null ? null : Text(itemData.title ?? '', style: itemData.titleStyle);
    final Text? subtitleText = itemData.subtitle == null ? null : Text(itemData.subtitle ?? '', style: itemData.subtitleStyle);
    final Widget? accesssoryWidget = _getAccessoryWidget(itemData, themeData);
    final Widget trailingWidget = accesssoryWidget ?? (itemData.customTrailing ?? const SizedBox());
    return FLListTile(
    ...
    switch (itemData.accessoryType) {
      ...
      case FLStaticListCellAccessoryType.accDetail:
        {
          ...
          if (itemData.accessoryString != null && (itemData.accessoryString?.length ?? 0) > 0) {
            return Row(
      ...
      case FLStaticListCellAccessoryType.accCheckBox:
        return Checkbox(
          value: itemData.accItemValue,
          onChanged: (v) {
            itemData.onChanged?.call(v ?? false);
          },
        );
    ...
    for (FLStaticSectionData sectionData in sections) {
      ...
      for (FLStaticItemData itemData in (sectionData.itemList ?? [])) {
        ...
        if (itemData.cellType == FLStaticListCellType.normal) {
          ...
          // customization
          itemCell = itemData.customizeContent ?? const SizedBox();
        }
